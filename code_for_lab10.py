...
def test_learn_play(d = 10, num_layers = 2, num_units = 100,
    ...
    if batch:
        qf = Q_learn_batch(game, q, iters=iters, episode_length = 100, n_episodes=10, eps=eps,
                           interactive_fn=interact)
    else:
        qf = Q_learn(game, q, iters=iters, interactive_fn=interact, eps=eps)
    if scores:
        # Plot learning curve
        scores, length = np.array([s[0] for s in scores]), np.array([s[1] for s in scores])
        ax = plot_points(scores, length, style='ro-')
        # Plot the running mean to smooth the learning curve
        plot_points(scores, np.cumsum(length) / np.arange(1, len(length) +1), style='bx-', ax=ax)
    for i in range(num_episodes):
        reward, _, animation = sim_episode(game, (episode_length if d > 5 else episode_length/2),
                                lambda s: greedy(qf, s), draw=(i == (num_episodes - 1)))
        print('Reward', reward)
    return animation
...
def try_value_iteration(d = 6, draw=True,
                        num_episodes = 1, episode_length = 100):
    return test_solve_play(d, draw, num_episodes, episode_length)

def try_tabular_q_learning(iters=10000, eps=0.5):
    return test_learn_play(iters=iters, tabular=True, batch=False, draw=True, num_episodes=10, eps=eps)

def try_tabular_batch_q_learning(iters=10, eps=0.5):
    return test_learn_play(iters=iters, tabular=True, batch=True, draw=True, num_episodes=10, eps=eps)

def try_nn_q_learning(iters=10000, eps=0.5):
    return test_learn_play(iters=iters, tabular=False, batch=False, draw=True, num_episodes=10, eps=eps)

def try_nn_batch_q_learning(iters=10, eps=0.5):
    return test_learn_play(iters=iters, tabular=False, batch=True, draw=True, num_episodes=10, eps=eps)
